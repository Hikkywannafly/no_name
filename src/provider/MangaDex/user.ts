...
import type { Includes, MangaContentRating, Order } from "./static";
...
export const getUserId = (id: string) => {
  ...
  if (id === "") {
    return Promise.reject(
      "ERROR - getUserId: Parameter `id` cannot be blank",
    );
  }
 ...
 */
export const getUserFollowedMangaFeed = (
  token: AuthenticationToken,
 ...
 */
export const getUserFollowedManga = (
  token: AuthenticationToken,
 ...
 */
export const getUserMe = (token: AuthenticationToken) => {
  const path = `/user/me`;
